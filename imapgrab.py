...
def imapgrab():
    ...
    ig_cmd.add_option(
        ...
        "--list",
        action="store_true",
        help="List the mailboxes available for download",
    )
    ig_cmd.add_option(
        "-d",
        "--download",
        action="store_true",
        help="Download mailboxes to separate files/folders",
    )
    ig_cmd.add_option(
        "-B",
        "--mbox",
        action="store_true",
        help="Download into Mbox format (optional, default)",
    )
    ig_cmd.add_option(
        "-M",
        "--maildir",
        action="store_true",
        help="Download into Maildir format (optional)",
    )
    ig_cmd.add_option(
        "-a",
        "--all",
        action="store_true",
        help="Force download all mail in a mailbox (optional)",
    )
    ig_cmd.add_option(
        "-S", "--ssl", action="store_true", help="Use SSL connection (optional)"
    )
    ig_cmd.add_option(
        "-s", "--server", dest="server", help="IP or domain of server (required)"
    )
    ig_cmd.add_option("-P", "--port", dest="port", help="Port of server (optional)")
    ig_cmd.add_option(
        "-u", "--username", dest="username", help="Username for account (required)"
    )
    ig_cmd.add_option(
        "-p", "--password", dest="password", help="Password for account (required)"
    )
    ig_cmd.add_option(
        "-m",
        "--mailboxes",
    ...
    ig_cmd.add_option(
        "-q", "--quiet", action="store_true", help="Don't display any output"
    )
    ...
    # verbose shows title
    if (
        ig_options.quiet is not True
        and ig_options.version is not True
        and ig_options.about is not True
    ):
    ...
    # check for list or download
        ...
        # --list, call list function
            ...
            if ig_options.quiet is not True:
...
def IG_backup_mail(ig_options):
    ...
    if ig_options.mailboxes is None:
        print(
            "ERROR: no mailboxes selected for download (use -m or --mailboxes option)"
        )
        return
    ...
    if ig_options.debug is True:
        print(
            'DEBUG_015: removed non-separating commas: "'
            + ig_download_str_rm_comma
            + '"'
        )
    ...
    if ig_options.folder is None:
        ...
        if ig_options.debug is True:
            print(
                "DEBUG_020: no -f parsed, using current directory: "
                + ig_options.folder
                + os.sep
                + "imapgrab"
            )
    ...
    if os.path.exists(ig_options.folder) is not True:
        ...
        if ig_options.debug is True:
            print(
                "DEBUG_021: folder does not exist, creating new folder in "
                + ig_options.folder
            )
        if ig_options.verbose is True:
        ...
        if ig_options.maildir is True:
            ...
            for f in maildir_directories:
                if os.path.isdir(fpath + os.sep + filename + os.sep + f) is not True:
                    if ig_options.verbose is True:
                        print(
                            "Maildir directory doesn't exist, creating \""
                            + fpath
                            + os.sep
                            + filename
                            + os.sep
                    ...
                    if ig_options.verbose is True:
                        print(
                            'Maildir directory already exists, using "'
                            + fpath
                            + os.sep
                            + filename
                            + os.sep
            ...
            if os.path.isfile(fpath + os.sep + filename + ".mbox") is not True:
                if ig_options.verbose is True:
                    print(
                        "Mbox file doesn't exist, creating \""
                        + fpath
                        + os.sep
                        + filename
                        + '.mbox"'
                ...
                if ig_options.verbose is True:
                    print(
                        'Mbox file already exists, using "'
                        + fpath
                        + os.sep
                        + filename
                        + '.mbox"'
    ...
    for mailbox in ig_download_list:
        ...
        if ig_options.verbose is True:
            print(
                'Calling getmail to retrieve mail for mailbox "'
                + ig_options.folder
                + os.sep
                + path_from_folder
                + os.sep
        ...
        if ig_options.verbose is True or ig_options.debug is True:
            ...
            subprocess.call(
                [cmd, "--quiet", "-g", ig_options.folder, "-r", rc_file_loc]
            )
